...
class PixCustomifyPlugin {
	 ...
	 */
	function output_dynamic_style( $editor = false ) {
		$custom_css = '';
		...
		foreach ( $this->options_list as $option_id => $option ) {
			...
			if ( isset( $option['css'] ) && ! empty( $option['css'] ) ) {
				// now process each
				$custom_css .= $this->convert_setting_to_css( $option_id, $option['css'], $editor );
			}
		...
		if ( ! empty( $this->media_queries ) ) {
			...
			foreach ( $this->media_queries as $media_query => $properties ) {
				...
				foreach ( $properties as $key => $property ) {
					...
					$custom_css .= "\t" . $this->proccess_css_property( $property_settings, $property_value, $editor ) . PHP_EOL;
				}
	...
	function output_typography_dynamic_style( $editor = false ) {
	 ...
	 * @return string
	 */
	protected function convert_setting_to_css( $option_id, $css_config = array(), $editor = false ) {
		$output = '';
		...
		foreach ( $css_config as $css_property ) {
			...
			if ( isset( $css_property['selector'] ) && isset( $css_property['property'] ) ) {
				$output .= $this->proccess_css_property( $css_property, $this_value, $editor ) . PHP_EOL;
			}
		}
	...
	protected function proccess_css_property( $css_property, $this_value, $editor = false ) {
		$unit = '';
